...
import { SMELT_OUTPUTS, type ItemStack, type ItemType } from '../items/types'
...
export interface SmelterInteriorHandle {
  bindings: SlotBinding[]
  slotVisuals: SlotVisual[]
  update: () => void
}

const SMELTER_CFG = BUILDINGS.smelter.smelting!

function isFuel(type: string): boolean {
  return SMELTER_CFG.isFuel(type)
}

function isSmeltable(type: string): boolean {
  return type in SMELTER_CFG.recipes
}

export function tickSmelterPlot(plotIndex: number, now: number) {
  const plot = state.plots[plotIndex]
  if (plot.built !== 'smelter') return
  tickSmeltingPlot(plot, plotIndex, now, SMELTER_CFG)
}

export const SMELTER_SMELT_DURATION_MS = SMELTER_CFG.cycleDurationMs

export function buildSmelterInterior(
  scene: Phaser.Scene,
  plotIndex: number,
  centerX: number,
  centerY: number,
  ...
  // Extra width consumed by additional output slots at higher levels. The
  // outputs stay centered on the original outputX; ore/fuel/arrow shift left
  // by half that extra width so the whole layout breathes.
  ...
  // Red-X cursor when hovering a slot with an item it can't accept.
  ...
  // One output slot per element in smelt.outputs. Slots stack vertically when
  // there are 2+; a single slot stays at the original outputY.
  const outputCount = smelt.outputs.length
  const outputLeftX = outputX - ((outputCount - 1) * OUTPUT_SLOT_GAP) / 2
  const isBarType = (t: ItemType) => SMELT_OUTPUTS.has(t)
  ...
  for (let i = 0; i < outputCount; i++) {
    ...
    binding.accepts = (t) => isBarType(t) && baseAccepts(t)
    binding.offer = (s) => isBarType(s.type) ? baseOffer(s) : 0
    dc.register(binding)
  ...
  const update = () => {
    ...
    tickSmelterPlot(plotIndex, now)

    if (smelt.cycleEndAt > 0 && getOre() && getFuel()) {
      const remaining = smelt.cycleEndAt - now
      const frac = Math.min(1, 1 - Math.max(0, remaining / SMELTER_CFG.cycleDurationMs))
