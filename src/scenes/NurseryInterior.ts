...
export function buildNurseryInterior(scene: Phaser.Scene): NurseryInteriorHandle {
  ...
  const BULK_W = 48
  const hasAnyBulk = NURSERY_ITEMS.some(e => ITEMS[e.type].maxStack > 1)
  ...
  scene.add.bitmapText(panelX, topRowY - ROW_H / 2 - 18, 'main', 'Nursery', FONT.title)
    .setOrigin(0.5, 0.5)
  ...
  const rowTexts: { entry: NurseryEntry; texts: Phaser.GameObjects.BitmapText[]; bulkTexts: Phaser.GameObjects.BitmapText[] }[] = []
